...
<form wire:submit.prevent="save" enctype="multipart/form-data">
    ...
    <div class="{{ $cmsUi['card'] ?? 'card' }}">
        <div class="{{ $cmsUi['card_body'] ?? 'card-body' }}">
            <h3>{{ $content->uuid ? __('cms::laravel_cms.edit_content') : __('cms::laravel_cms.create_content') }}</h3>
            ...
            @if(in_array('category_uuid', $fields, true))
                <div class="{{ $cmsUi['form_group'] ?? 'mb-3' }}">
                    <label class="{{ $cmsUi['label'] ?? '' }}" for="category_uuid">{{ __('cms::laravel_cms.category') }}</label>
            ...
            @if(in_array('media', $fields, true))
                ...
                <div class="{{ $cmsUi['form_group'] ?? 'mb-3' }}">
                    ...
                    @elseif($content->video_poster_path)
                        <div><img src="{{ $content->video_poster_path }}" alt="{{ __('cms::laravel_cms.poster') }}" style="max-width: 240px;"></div>
                    @elseif($content->avatar_path)
                        <div><img src="{{ $content->avatar_path }}" alt="{{ __('cms::laravel_cms.avatar') }}" style="max-width: 240px;"></div>
                    @endif
                </div>
                ...
                <div class="{{ $cmsUi['form_group'] ?? 'mb-3' }}">
                    ...
                    @elseif($content->small_video_poster_path)
                        <div><img src="{{ $content->small_video_poster_path }}" alt="{{ __('cms::laravel_cms.small_poster') }}" style="max-width: 240px;"></div>
                    @elseif($content->small_avatar_path)
                        <div><img src="{{ $content->small_avatar_path }}" alt="{{ __('cms::laravel_cms.small_avatar') }}" style="max-width: 240px;"></div>
                    @endif
                </div>
                ...
                <div class="{{ $cmsUi['card'] ?? 'card' }}" style="margin-bottom: 1rem;" wire:key="locale-{{ $locale }}">
                    ...
                    <div class="{{ $cmsUi['card_body'] ?? 'card-body' }}">
                        ...
                        @if(in_array('short_description', $fields, true))
                            <div class="{{ $cmsUi['form_group'] ?? 'mb-3' }}">
                                <label class="{{ $cmsUi['label'] ?? '' }}" for="short_description_{{ $locale }}">{{ __('cms::laravel_cms.short_description') }}</label>
            ...
            @if(in_array('external_url', $fields, true))
                <div class="{{ $cmsUi['form_group'] ?? 'mb-3' }}">
                    <label class="{{ $cmsUi['label'] ?? '' }}" for="external_url">{{ __('cms::laravel_cms.external_url') }}</label>
        ...
        <div class="{{ $cmsUi['card_footer'] ?? 'card-footer text-end' }}">
            <a class="{{ $cmsUi['button_secondary'] ?? 'btn btn-outline-secondary' }}" href="{{ route($routePrefix . 'content.index') }}" wire:navigate>{{ __('cms::laravel_cms.cancel') }}</a>
